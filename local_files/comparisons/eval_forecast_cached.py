...
models_to_eval = ['base', 'fine_val']

for init_date in tqdm(initialization_dates, desc="Evaluating Forecasts"):
    for model_name in tqdm(models_to_eval, desc="MSE & Diff Calc"):
        predictions = xr.open_dataset(f"/Datastorage/saptarishi.dhanuka_asp25/rolled_out_preds/{model_name}_init_{init_date.strftime('%Y%m%d')}.ncs")
