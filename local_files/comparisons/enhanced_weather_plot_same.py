...
def plot_forecast_with_ground_truth(csv_path: str, target_files_dir: str, output_path: str, 
    ...
    # Sum precipitation values across all init dates for each lead time
    if not target_df.empty:
        target_summary = (
            target_df.groupby(['lead_time_hours'])['precipitation']
              .sum()
              .reset_index()
              .rename(columns={'precipitation': 'total_precip'})
        )
        print(f"Calculated total precipitation for {len(target_summary)} lead times")

    print("\n--- Forecast Summary Statistics ---")
    print(summary_df.head())
    if not target_df.empty:
        print("\n--- Ground Truth Total Precipitation by Lead Time ---")
    ...
    # Create two panels: MSE with precipitation overlay, and % improvement
    fig, (ax_top, ax_bottom) = plt.subplots(
        nrows=2, sharex=True, figsize=(14, 10),
        gridspec_kw={'height_ratios': [2, 1]}
    )
    
    # Create secondary y-axis for precipitation on the top plot
    if not target_df.empty:
        ax_precip = ax_top.twinx()
    ...
    ax_top.set_title('Forecast Skill (MSE) vs. Lead Time with Total Precipitation', fontsize=16, pad=20)
    ...
    # Plot precipitation on secondary axis
    if not target_df.empty:
        ax_precip.plot(
            target_summary['lead_time_hours'],
            target_summary['total_precip'],
            color='tab:green', linewidth=3, alpha=0.7,
            label='Total Precipitation', marker='s', markersize=6
        )
        ax_precip.set_ylabel('Total Precipitation (mm)', fontsize=12, color='tab:green')
...
if __name__ == "__main__":
    ...
    #  add argument for the year to parse
...
"""
Example usage:
python enhanced_weather_plot_same.py \
    ...
    --model_rename "Graphcast_Base:Base,Graphcast_Finetuned1:Finetuned1, HRES:HRES" \
