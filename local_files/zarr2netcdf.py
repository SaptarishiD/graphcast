...
def zarr_to_netcdf(zarr_path, output_path=None, use_dask=False, verbose=False, encoding=False, encoding_level=3):
    ...
    if use_dask:
        with ProgressBar():
            ds.to_netcdf(output_path, compute=True, encoding=encoding_dict, format='NETCDF4', engine='netcdf4')
    else:
        ds.to_netcdf(output_path, encoding=encoding_dict, format='NETCDF4', engine='netcdf4')
